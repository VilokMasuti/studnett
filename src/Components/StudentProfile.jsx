...
import { submitStudentProfile } from "../api/api"
...
const StudentProfile = () => {
  ...
  //  WEB-ONLY: Audio recording with MediaRecorder API
  // Swap this logic for mobile: Use react-native-audio-recorder-player in React Native!



  useEffect(() => {
    const AudioRecordng = async () => {
      try {
        // In React Native, use expo-av or react-native-audio-recorder-player for this!
  ...
  // These can stay almost the same in React Native --- START STOP CLEAR

  const startRecording = () => {
    // In React Native, call startAsync() or similar from native lib
  ...
  const stopRecording = () => {
    // In React Native, call stopAsync() or similar from native lib
    if (mediaRecorderRef.current) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      // --- Compress audio before submission ---
      // Web: Use lamejs
      // React Native: Use native module compression or send to backend
      ...
      formData.append("interestCategory", data.interestCategory);
      formData.append("subcategory", data.subcategory);
      formData.append("introduction", data.introduction);
      formData.append("audioFile", compressedAudio, "intro.mp3");
      // Submit to endpoint (mock)
  ...
  return (
    <section className="min-h-screen w-full flex flex-col lg:flex-row font-sans ">
      ...
      <div className="flex-1 flex items-center justify-center    p-2 md:p-4">
        <div className="w-full max-w-lg md:max-w-2xl  duration-700  border border-amber-300   rounded-3xl p-4 md:p-8 shadow-2xl">
          {/* Header */}
              ...
              {audioUrl && (
                <div className="w-full flex flex-col gap-2 mt-2">
                  ...
                  <button
                    type="button"
                    onClick={clearAudio}
                    className="rounded-lg py-2 xl:w-[20rem] xl:ml-[21%] mt-2 font-semibold border border-black  bg-black  text-white transition  font-sans cursor-pointer  lg:ml-[18%] lg:w-[15rem] md:ml-[22%]  md:w-[19rem]"
                  >
                    Clear Recording
                  </button>
                </div>
